...
5. Then decide whether there is ONE clean new entry worth taking from the watchlist right now, informed by news AND the multi-timeframe indicator read. Match the horizon to the thesis (intraday vs multi-day swing) — do not force a same-day mindset. If yes, place a market buy sized at or under \$$MAX_ORDER_USD and say why (news + levels). If nothing is clean, take no new trade — say 'no new trade' and why.
